...
RUN EZA_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-unknown-linux-musl" || echo "aarch64-unknown-linux-gnu") && \
    EZA_VERSION=$(curl -s https://api.github.com/repos/eza-community/eza/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
    curl -fsSL "https://github.com/eza-community/eza/releases/download/v${EZA_VERSION}/eza_${EZA_ARCH}.tar.gz" \
    | tar xz && mv eza_* eza 2>/dev/null || true
 
# xh
RUN XH_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-unknown-linux-musl" || echo "aarch64-unknown-linux-musl") && \
    XH_VERSION=$(curl -s https://api.github.com/repos/ducaale/xh/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
...
RUN WE_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-unknown-linux-musl" || echo "aarch64-unknown-linux-musl") && \
    WE_VERSION=$(curl -s https://api.github.com/repos/watchexec/watchexec/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
...
RUN SD_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-unknown-linux-musl" || echo "aarch64-unknown-linux-musl") && \
    SD_VERSION=$(curl -s https://api.github.com/repos/chmln/sd/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
...
RUN DUST_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-unknown-linux-musl" || echo "aarch64-unknown-linux-gnu") && \
    DUST_VERSION=$(curl -s https://api.github.com/repos/bootandy/dust/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
...
RUN PROCS_ARCH=$([ "$TARGETARCH" = "amd64" ] && echo "x86_64-linux" || echo "aarch64-linux") && \
    PROCS_VERSION=$(curl -s https://api.github.com/repos/dalance/procs/releases/latest | jq -r '.tag_name' | sed 's/^v//') && \
...
RUN pip install --break-system-packages --no-cache-dir semgrep
